...
class CartItemCollectionViewCell: UICollectionViewCell {
    ...
    private func viewComponents() {
        ...
        removeCellButton.addTarget(self, action: #selector(closeButtonTapped), for: .touchUpInside)
    }
    
    @objc private func addButtonTapped() {
        quantityNumber += 1
        quantityCount.text = String(quantityNumber)
        guard let prodName = itemName.text else {
            return
        }
        delegate?.quantityUpdated(productName: prodName, quantity: quantityNumber)
    }
    ...
    @objc private func closeButtonTapped() {
        ...
        delegate?.closeButtonTapped(productName: prodName)
    }
}
