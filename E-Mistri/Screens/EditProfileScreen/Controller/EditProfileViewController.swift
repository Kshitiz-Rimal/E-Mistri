...
extension EditProfileViewController: EditProfileTableViewCellDelegate {
    ...
    func tappedEditProfileButton(isOkay: Bool, userName name: String, phoneNumber phoneNum: String, address: String) {
        if isOkay {
            let getResponse = { postResponse in
                print("API response:::::::", postResponse)
            }
            editProfile(completion: getResponse,
                       userName: name,
    ...
    func editProfile(completion: @escaping (EditProfileResponse) -> Void,
                     userName: String,
                     phoneNumber: String,
                     address: String) {
        ...
        let body = Customer(fullName: userName,
        ...
        let profilePicture = profileImage.jpegData(compressionQuality: 0.7)!
            ...
            case .success(let postResponse):
                ...
                AF.upload(
                    multipartFormData: { (multiFormData) in
                        multiFormData.append(profilePicture,
                                             withName: "display_picture",
                                             fileName: "displayPicture",
                                             mimeType: "image/*")
                    }, to: ApiURL.editDisplayPictureURL,
                    method: .post,
                    headers: header).validate().responseDecodable(of: DisplayPicturePathResponse.self ) { imgResponse in
                        switch imgResponse.result {
                        case .success:
                            guard let displayPicturePath = imgResponse.value?.displayPicturePath else {
                                print("No RESPONSE:::::::")
                                return
                            }
                            let changedDisplaypicturePath = displayPicturePath.replacingOccurrences(
                                of: "http://localhost:3000",
                                with: ApiURL.baseURL
                            )
                            strongSelf.userData.userProfilePicturePath = changedDisplaypicturePath
